...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        actions: [
          IconButton(onPressed: ()=> nextScreen(context, const SearchPage()), icon: const Icon(Icons.search),),
        ],
        title: const Text("Groups", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20,),),
        centerTitle: true,
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              onTap: ()=> nextScreenReplace(context, ProfilePage(userName: userName, email: email)),
              selectedColor: Theme.of(context).primaryColor,
              contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5),
              leading: const Icon(Icons.group),
                  ...
                  onPressed: () async {
                    if (groupName != "") {
                      ...
                      DatabaseService(uid: FirebaseAuth.instance.currentUser!
                          .uid).createGroup(userName, FirebaseAuth.instance
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text("You've not join any group, tap on the add icon and create or join any group by search button.", textAlign: TextAlign.center,),
        ],
      ),
    );
  }
